...
Partial Class Summary_BCYMovement
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            db.sub_ExecuteNonQuery("Delete from Temp_Modify_Assessment_Domestic Where UserID=" & Session("UserId_BondCFS") & "")
    ...
    Protected Sub btnSave_Click(sender As Object, e As EventArgs)
        ...
        Try
            Dim dblassessno As String = ""
            strSql += "select isnull(max(Assessno),0)+1 from Domestic_assessm"
            dt = db.sub_GetDatatable(strSql)
            ...
            txtworkyear.Text = strWorkyear
            strSql = ""
            strSql = "USP_COPY_INSERT_AssessM'" & Trim(txtinvno.Text & "") & "','" & Trim(txtworkyear.Text & "") & "','" & Session("UserId_BondCFS") & "'"
            dt = db.sub_GetDatatable(strSql)
            btnSave.Text = "Copy"
            btnSave.Attributes.Add("Class", "btn btn-primary")
            lblSession.Text = "Invoice No : " & dblassessno & " generated successfully"
    ...
    End Sub
        ...
        Try
            Clear()          
            strSql = ""
            strSql += "USP_modify_assessment_assessno_textchanged_BOND '" & Trim(txtinvno.Text & "") & "','" & Session("UserId_BondCFS") & "','" & Trim(txtworkyear.Text) & "'"
    ...
    Public Sub Clear()
        Try            
            ...
            db.sub_ExecuteNonQuery("Delete from Temp_Modify_Assessment_DOMESTIC Where UserID=" & Session("UserID") & "")
            FillGrid()
        Catch ex As Exception
            lblError.Text = "Error in procedure: " & Reflection.MethodBase.GetCurrentMethod.Name & ". " & ex.Message.ToString
        End Try
    ...
    Public Sub FillGrid()
        Try
            strSql = ""
            strSql += "USP_fetch_from_temp_modify_assessment_Domestic '" & Session("UserId_BondCFS") & "'"
            ds = db.sub_GetDataSets(strSql)
    ...
    Protected Sub lnkadd_Click(sender As Object, e As EventArgs) Handles lnkadd.Click
        Try
            strSql = ""
            strSql += "select * from Temp_Modify_Assessment_DOMESTIC where IsCancel=0 and AccountID='" & Trim(ddlaccntheads.SelectedValue) & "' and UserID='" & Session("UserID") & "' and ContainerNo='" & Trim(txtContainerNo.Text) & "'"
            dt = db.sub_GetDatatable(strSql)
            ...
            strSql += "'" & Format(Val(txtamount.Text) * (dblIGST / 100), "0.00") & "','" & dbltaxgroupid & "','" & Session("UserID") & "','" & Trim(txtContainerNo.Text) & "'"
            db.sub_ExecuteNonQuery(strSql)

...
Partial Class Summary_BCYMovement
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            db.sub_ExecuteNonQuery("Delete from Temp_Charges Where UniqueID=" & Session("UserId_BondCFS") & "")
            Filldropdown()
            Add(sender, e)
            If Not (Request.QueryString("TariffIDView") = "") Then
                TariffID = ed.Decrypt(HttpUtility.UrlDecode(Request.QueryString("TariffIDView")))
                strSql = ""
                strSql = "use_view_cancel '" & TariffID & "','" & Session("UserId_BondCFS") & "'"
    ...
    Protected Sub Filldropdown()
        Try
            dt = db.sub_GetDatatable("exec get_sp_table 'Bond_tariffmaster' ,'TariffID,EntryID','','TariffID'")
            If (dt.Rows.Count > 0) Then
            ...
            dt = db.sub_GetDatatable("exec get_sp_table 'Bond_accountmaster','AccountName,AccountID','','AccountID'")
            If (dt.Rows.Count > 0) Then
            ...
            End If
            dt = db.sub_GetDatatable("exec get_sp_table 'bond_Chargesmaster','ChargesID,ChargesBased','','ChargesID'")
            If (dt.Rows.Count > 0) Then
            ...
            End If
            strSql = ""
            strSql += "USP_LOCATION_CUSTOMER_WISE '" & Trim(ddltraiff.SelectedItem.Text & "") & "','Bond'"
            dt = db.sub_GetDatatable(strSql)
            ...
            dt = db.sub_GetDatatable("exec get_sp_table 'CargoType','Cargotypeid,Cargotype','','Cargotypeid'")
            If (dt.Rows.Count > 0) Then
                ddlcargoType.DataSource = dt
    ...
    Protected Sub btnSave_Click(sender As Object, e As EventArgs)
        Try
            For Each row As GridViewRow In grdcontainer.Rows
                strSql = ""
                strSql += "usp_insert_into_bond_tariff"
                ...
                strSql += "'" & Trim(CType(row.FindControl("lblAccountHead"), Label).Text) & "','" & Trim(CType(row.FindControl("lblsize"), Label).Text) & "','" & Trim(CType(row.FindControl("lblchargesBased"), Label).Text) & "',"
                ...
                strSql += ",'" & Trim(CType(row.FindControl("lblconsarea"), Label).Text) & "','" & Trim(CType(row.FindControl("lblLocationID"), Label).Text) & "','" & Trim(CType(row.FindControl("lblSlabID"), Label).Text) & "','" & Trim(CType(row.FindControl("lblCargoID"), Label).Text) & "','" & Trim(CType(row.FindControl("lblIsInternal"), Label).Text) & "'"
                dt = db.sub_GetDatatable(strSql)                
            Next
            Button1.Text = "Save"
    ...
    Protected Sub ddltraiff_SelectedIndexChanged(sender As Object, e As EventArgs)
        Try
            strSql = ""
            strSql = "exec get_sp_table 'bond_tariffmaster','TariffID, Convert(VARCHAR(50),Convert(Datetime, EffectiveFrom),105) as Efrom,Convert(VARCHAR(50),Convert(Datetime, EffectiveUpto),105) as Upto,TariffDescription ','EntryID=''" & Trim(ddltraiff.SelectedValue & "") & "'' ','TariffID'"
            dt = db.sub_GetDatatable(strSql)
            If (dt.Rows.Count > 0) Then
    ...
    Protected Sub Button1_Click(sender As Object, e As EventArgs)
        Try
            ...
            strSql = ""
            strSql += "use_temp_traiff_setting "
            strSql += "'" & Trim(ddltraiff.SelectedItem.Text & "") & "','" & Trim(ddlbond.SelectedItem.Text) & "','" & Trim(txtfrom.Text & "") & "','" & Trim(txtUpTo.Text & "") & "',"
            strSql += "'" & Trim(ddlAccount.SelectedItem.Text & "") & "','" & Trim(ddlsize.SelectedValue & "") & "','" & Trim(ddlCharges.SelectedItem.Text & "") & "',"
            strSql += "'" & Trim(txtfixed.Text & "") & " ','" & IsTax & "','" & OnDelivereed & "','" & Session("UserId_BondCFS") & "','" & chkconsarea.Checked & "','" & ddlLocation.SelectedValue & "','" & Val(ddlSlab.SelectedValue) & "','" & Trim(ddlcargoType.SelectedValue & "") & "','" & chkIsInternal.Checked & "'"
            dt = db.sub_GetDatatable(strSql)
            Add(sender, e)
    ...
    Protected Sub Add(sender As Object, e As EventArgs)
        Try
            strSql = "USP_FETCH_DATA_FROM_TEMP_CHARGES '" & Session("UserId_BondCFS") & "'"
            dt = db.sub_GetDatatable(strSql)
            grdcontainer.DataSource = dt
            grdcontainer.DataBind()
        Catch ex As Exception
    ...
    Protected Sub lnkDelete_Click(sender As Object, e As EventArgs)
        Try
            ...
            dt = db.sub_GetDatatable("USP_Delete_temp_bond_traiff '" & AutoID & "','" & Session("UserId_BondCFS") & "'")
            Add(sender, e)
            If (dt.Rows.Count > 0) Then
            End If
    ...
    Protected Sub ddlCharges_SelectedIndexChanged(sender As Object, e As EventArgs)
        Try
            If ddlCharges.SelectedValue = 2 Then
            ...
            ElseIf ddlCharges.SelectedValue = 11 Or ddlCharges.SelectedValue = 12 Or ddlCharges.SelectedValue = 7 Then
                strSql = ""
                strSql += "select distinct slabID FROM bond_slabs  order by slabID "
                dt = db.sub_GetDatatable(strSql)
    ...
    Protected Sub btnNOCList_Click(sender As Object, e As EventArgs)
        Try
            strSql = ""
            strSql += "select * from TEMP_TARIFF_SEARCH where userid='" & Session("UserId_BondCFS") & "'"
            dt = db.sub_GetDatatable(strSql)
            If dt.Rows.Count > 0 Then
    ...
    Protected Sub ddlSlab_SelectedIndexChanged(sender As Object, e As EventArgs)
        Try
            ...
            Else
                ...
                strSql = ""
                strSql += "select * FROM bond_slabs WHERE SlabID=" & ddlSlab.SelectedValue & " order by FromSlab"
                dt = db.sub_GetDatatable(strSql)

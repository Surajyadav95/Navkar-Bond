...
Partial Class Summary_BCYMovement
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            db.sub_ExecuteNonQuery("Delete from Temp_Other_Assessment Where UserID=" & Session("UserId_BondCFS") & "")
            ...
            txtStorageFrom.Text = Convert.ToDateTime(Now).ToString("yyyy-MM-dd")
            txtStorageUpto.Text = Convert.ToDateTime(Now).ToString("yyyy-MM-dd")
            txtInsFrom.Text = Convert.ToDateTime(Now).ToString("yyyy-MM-dd")
            txtInsUpto.Text = Convert.ToDateTime(Now).ToString("yyyy-MM-dd")
    ...
    Protected Sub btnSave_Click(sender As Object, e As EventArgs)
        Try
            ...
            '    Exit Sub
            'End If
            If (lblIsTax.Text = "1" Or lblIsTax.Text = "") Then
                Call Sub_SGTRate()
            Else
                dbltaxgroupid = 11
            End If
            strSql = ""
            strSql += "USP_save_part_for_noc_assessment_Other '" & strWorkyear & "','" & Trim(ddlInvoiceType.SelectedItem.Text & "") & "','" & Trim(ddlwarehouse.SelectedItem.Text & "") & "'"
            ds = db.sub_GetDataSets(strSql)
            If ds.Tables(0).Rows(0)(0) = 0 Then
                dblassessno = 1
            Else
                dblassessno = Val(ds.Tables(0).Rows(0)(0)) + 1
            End If
            If ds.Tables(1).Rows.Count > 0 Then
                ScriptManager.RegisterStartupScript(Me, Me.GetType, "Key", "alert('Invoice No " & ds.Tables(1).Rows(0)(0) & " already generated. Cannot proceed!');", True)
                Exit Sub
            End If
            For Each row In grdcharges.Rows
                strSql = ""
                strSql += "USP_insert_into_bond_assessd_other_assessment '" & dblassessno & "','" & strWorkyear & "','" & Val(CType(row.FindControl("lblaccntid"), Label).Text) & "',"
                ...
                strSql += "'" & dbltaxgroupid & "','" & Convert.ToDateTime(txtStorageFrom.Text).ToString("yyyy-MM-dd") & "','" & Convert.ToDateTime(txtStorageUpto.Text).ToString("yyyy-MM-dd") & "',"
                strSql += "'" & Convert.ToDateTime(txtInsFrom.Text).ToString("yyyy-MM-dd") & "','" & Convert.ToDateTime(txtInsUpto.Text).ToString("yyyy-MM-dd") & "','" & Trim(txtnocno.Text & "") & "'"
            ...
            End If
            strSql = ""
            strSql += "USP_insert_into_bond_assessm_other_assessment '" & dblassessno & "','" & strWorkyear & "','" & Trim(ddlCHA.SelectedValue) & "','" & Trim(ddlcustomer.SelectedValue) & "','" & Trim(ddlimporter.SelectedValue) & "',"
            strSql += "'" & Convert.ToDateTime(txtinvdate.Text).ToString("yyyy-MM-dd") & "','" & Convert.ToDateTime(txtvaliddate.Text).ToString("yyyy-MM-dd") & "','" & Convert.ToDateTime(txtInsUpto.Text).ToString("yyyy-MM-dd") & "',"
            strSql += "'" & Trim(txtnocno.Text & "") & "','','" & Trim(txtbeno.Text) & "','','',"
            strSql += "'','0','0','" & dblSumNetAmtTotal & "','" & dblSumNetAmtTotal & "','" & Session("UserId_BondCFS") & "',"
            strSql += "'" & dblSumSGSTAmt & "','" & dblSumCGSTAmt & "','" & dblSumIGSTAmt & "','" & dblgrandtotal & "','" & lblpartyid.Text & "','" & Trim(txtbondno.Text) & "','" & Trim(txtremarks.Text & "") & "','" & Trim(ddlInvoiceType.SelectedItem.Text & "") & "','" & Trim(ddlwarehouse.SelectedItem.Text & "") & "','" & Trim(lblExDepositeNo.Text & "") & "'"
            db.sub_ExecuteNonQuery(strSql)
            Clear()
            txtinvdate.Text = Convert.ToDateTime(Now).ToString("yyyy-MM-dd")
            txtnocno.Text = ""
    ...
    Public Sub FillGrid()
        Try
            strSql = ""
            strSql += "USP_fill_grid_other_assessment '" & Session("UserId_BondCFS") & "'"
            ds = db.sub_GetDataSets(strSql)
    ...
    Protected Sub lnkadd_Click(sender As Object, e As EventArgs) Handles lnkadd.Click
        Try
            ...
            strSql = ""
            strSql += "select * from temp_other_assessment where IsCancel=0 and AccountID='" & Trim(ddlaccntheads.SelectedValue) & "' and UserID='" & Session("UserId_BondCFS") & "'"
            dt = db.sub_GetDatatable(strSql)
            If dt.Rows.Count > 0 Then
            ...
            End If
            strSql = ""
            strSql += "USP_insert_into_temp_other_assessment '" & Trim(ddlaccntheads.SelectedValue) & "','" & Trim(txtamount.Text & "") & "','" & Session("UserId_BondCFS") & "'"
            db.sub_ExecuteNonQuery(strSql)
            If ddlInvoiceType.SelectedValue = "Other" Then
                dblAmount += Val(txtamount.Text)
            End If
            lblTaxAmount.Text = dblAmount
            If Not ddlInvoiceType.SelectedValue = "Transport" Then
                Call Sub_SGTRate()
            End If
            FillGrid()
    ...
    Protected Sub lnkCancel_Click(sender As Object, e As EventArgs)
        Try
            ...
            strSql = ""
            strSql += "Update Temp_Other_Assessment set IsCancel=1 where accountid='" & AccountId & "'"
            db.sub_ExecuteNonQuery(strSql)
            Sub_SGTRate()
            FillGrid()
        Catch ex As Exception
    ...
    Protected Sub txtnocno_TextChanged(sender As Object, e As EventArgs) Handles txtnocno.TextChanged
        Try
            Clear()
            strSql = ""
            strSql += "USP_NOC_TEXT_CHANGED_OTHER_ASSESSMENT '" & Trim(txtnocno.Text & "") & "'"
            dt = db.sub_GetDatatable(strSql)
            If dt.Rows.Count > 0 Then
    ...
    Protected Sub btnIndentlist_Click(sender As Object, e As EventArgs)
        Try
            strSql = ""
            strSql += "select * from Temp_Other_Search_NOC where USERID=" & Session("UserId_BondCFS") & ""
            dt = db.sub_GetDatatable(strSql)
            If dt.Rows.Count > 0 Then
                txtnocno.Text = Trim(dt.Rows(0)("NOCNO") & "")
                txtExBoeNo.Text = Trim(dt.Rows(0)("ExBoeNo") & "")
                lblExDepositeNo.Text = Trim(dt.Rows(0)("ExDepositeNo") & "")

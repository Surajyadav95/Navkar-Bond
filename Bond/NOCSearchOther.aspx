<%@ Page Title="Bond" Language="VB" EnableEventValidation="false" MasterPageFile="~/Bond/PopUp.master" AutoEventWireup="false" CodeFile="NOCSearchOther.aspx.vb" Inherits="Account_ItemList" Culture="en-GB" %>
...
window.opener.document.getElementById("MainContent_btnIndentlist").click();
self.close();
}
</script>
...
<asp:DropDownList runat="server" ID="ddlSearchCriteria" CssClass="form-control">
    ...
    <asp:ListItem Value="5">Ex BOE No</asp:ListItem>
</asp:DropDownList>
</div>
</div>
...
<div class="col-sm-1 col-xs-1" style="padding-left:16px;">
...
<asp:BoundField DataField="BOENo" HeaderText="BOE No"></asp:BoundField>
    <asp:TemplateField HeaderText="Ex BOE No" HeaderStyle-CssClass="text-center">
        <ItemTemplate>
            <asp:Label runat="server" ID="lblExBoeNo" text='<%#Eval("exboeno")%>'></asp:Label>
        </ItemTemplate>
...
<asp:BoundField DataField="BondType" HeaderText="Bond Type"></asp:BoundField>
    <asp:TemplateField HeaderText="Ex Deposite No" HeaderStyle-CssClass="text-center">
        <ItemTemplate>
            <asp:Label runat="server" ID="lblExDepositNo" text='<%#Eval("ENTRYID")%>'></asp:Label>
        </ItemTemplate>
        <ItemStyle HorizontalAlign="Center" />
    </asp:TemplateField>
</Columns>
...
<script type="text/javascript" >
function callparentfunction() {
window.opener.document.getElementById("ContentPlaceHolder1_btnIndentlist").click();
self.close();
}
</script>    
</asp:Content>

...
Partial Class Summary_BCYMovement
    ...
    Protected Sub Filldropdown()
        Try
            dt = db.sub_GetDatatable("USP_tariff_List_get")
            If dt.Rows.Count > 0 Then
    ...
    Protected Sub btnSave_Click(sender As Object, e As EventArgs)
        Try
            If (btnSave.Text = "Update") Then
                strSql = ""
                strSql += "usp_tariff_master'" & Trim(ddlCustomer.SelectedValue & "") & "','" & Trim(txttariffId.Text & "") & "','" & Trim(txtDescription.Text & "") & "',"
                strSql += "'" & Convert.ToDateTime(Trim(txtfrom.Text & "")).ToString("yyyyMMdd") & "','" & Convert.ToDateTime(Trim(txtTo.Text & "")).ToString("yyyyMMdd") & "',"
                strSql += "'" & Trim(txtDiscount.Text & "") & "','" & Trim(chkisActive.Checked & "") & "','" & Session("UserId_BondCFS") & "'," & Val(txtStorageWeeks.Text) & "," & Val(txtInsuranceWeeks.Text) & ""
                dt = db.sub_GetDatatable(strSql)
            ...
            Else
                ...
                strSql = ""
                strSql = "SELECT * FROM bond_tariffmaster WHERE CustID='" & Val(ddlCustomer.SelectedValue) & "'"
                dt = db.sub_GetDatatable(strSql)
                If (dt.Rows.Count > 0) Then
                    ddlCustomer.Text = "0"
    ...
    Protected Sub txttariffId_TextChanged(sender As Object, e As EventArgs)
        Try
            strSql = ""
            strSql = "select  Convert(VARCHAR(50),Convert(Datetime, EffectiveFrom),105) EffectiveF,Convert(VARCHAR(50),Convert(Datetime, EffectiveUpto),105) EffectiveT,* from bond_tariffmaster where  TariffID ='" & txttariffId.Text & "'"
            dt = db.sub_GetDatatable(strSql)
            If dt.Rows.Count > 0 Then
                txtDescription.Text = Trim(dt.Rows(0)("TariffDescription"))
                txtDiscount.Text = Trim(dt.Rows(0)("discPercent"))

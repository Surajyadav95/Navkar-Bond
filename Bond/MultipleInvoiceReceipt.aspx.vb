...
Partial Class Summary_BCYMovement
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            Filldropdown()
            getLastReceiptNo()
            db.sub_ExecuteNonQuery("Delete from Temp_Receipt_Multiple Where UniqueID=" & Session("UserId_BondCFS") & "")
            db.sub_ExecuteNonQuery("Delete from Temp_Receipt_Multiple_Invoices Where Addedby=" & Session("UserId_BondCFS") & "")
    ...
    Protected Sub Filldropdown()
        Try
            ...
            ddlcustomer.DataSource = ds.Tables(5)
    ...
    Protected Sub btnsearch_Click(sender As Object, e As EventArgs)
        Try
            ClearForm()
            strSql = ""
            strSql += "USP_MULTIPLEINVOICE_ImportPDADJUSTMENT_NEW '" & Trim(txtpdcode.Text & "") & "','" & Session("UserId_BondCFS") & "'"
    ...
    Protected Sub btnAdd_Click(sender As Object, e As EventArgs)
        Try
            ...
            strSql += "'" & Convert.ToDateTime(Trim(txtmodedate.Text & "")).ToString("yyyy-MM-dd") & "','" & Session("UserId_BondCFS") & "','" & Replace(Trim(txtModeNo.Text), "'", "''") & "'"
            dt = db.sub_GetDatatable(strSql)
    ...
    Protected Sub Receipt(sender As Object, e As EventArgs)
        Try
            'Dim dt As DataTable
            ds = db.sub_GetDataSets("usp_select_tmp_Receipt_multiple '" & Session("UserId_BondCFS") & "'")
    ...
    Protected Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        Try
            ...
            strSql += "" & Trim(ddlpayment.SelectedValue) & "," & Val(ddlfill.SelectedValue) & ",'" & 0 & "','" & Replace(Trim(txtremarks.Text), "'", "''") & "',"
    ...
    Public Sub ClearForm()
        Try
            ...
            txtremarks.Text = ""
            strSql = ""
            strSql += "select *,convert(nvarchar(10),InvoiceDate,25 ) as [Invoice Date] from Temp_Receipt_Multiple_Invoices where iscancel=0 and addedby='" & Session("UserId_BondCFS") & "'"
            dt = db.sub_GetDatatable(strSql)
            If dt.Rows.Count > 0 Then
                grdinvoices.DataSource = dt
                grdinvoices.DataBind()
    ...
    Protected Sub ddlcustomer_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ddlcustomer.SelectedIndexChanged
        Try
            If ddlcustomer.SelectedValue <> 0 Then
                strSql = ""
                strSql += "SELECT distinct PDCode FROM Bond_PDAccounts WHERE AgencyName like '%" & Trim(ddlcustomer.SelectedItem.Text) & "%'"
                dt = db.sub_GetDatatable(strSql)
                If dt.Rows.Count > 0 Then
    ...
    Protected Sub txtreceivedamt_TextChanged(sender As Object, e As EventArgs)
        Try
            ...
            If CType(gr.FindControl("chkright"), CheckBox).Checked = True Then
                ...
                strSql += "'" & Trim(CType(gr.FindControl("lblnetamtrec"), Label).Text) & "','" & Trim(CType(gr.FindControl("lblOS"), Label).Text) & "','" & Session("UserId_BondCFS") & "','" & Trim(CType(gr.FindControl("lblautoid"), Label).Text) & "','" & Trim(CType(gr.FindControl("chkright"), CheckBox).Checked) & "'"
                db.sub_ExecuteNonQuery(strSql)
            End If
            UpdatePanel1.Update()
            UpdatePanel2.Update()
    ...
    Protected Sub Button1_ServerClick(sender As Object, e As EventArgs)
        Try
            ...
            Next
            strSql = ""
            strSql += "Select * from bond_assessM where AssessNo='" & Trim(txtinvoiceadd.Text & "") & "' AND IsCancel=0"
            dt = db.sub_GetDatatable(strSql)
            ...
            'End If
            strSql = ""
            strSql += "USP_MULTIPLEINVOICE_INSERT_EXTRA_INVOICE_INTO_TEMP '" & Trim(txtinvoiceadd.Text & "") & "','" & Session("UserId_BondCFS") & "'"
            db.sub_ExecuteNonQuery(strSql)
            FillGrid()
            txtinvoiceadd.Text = ""
            UpdatePanel2.Update()

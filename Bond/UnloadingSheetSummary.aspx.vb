...
Partial Class Summary_BCYMovement
    ...
    Protected Sub btnSave_Click(sender As Object, e As EventArgs)
        Try
            strSql = ""
            strSql += " USP_UNLOADING_SHEET_SEARCH '" & Convert.ToDateTime(txtfromDate.Text).ToString("yyyy-MM-dd") & "','" & Convert.ToDateTime(txttoDate.Text).ToString("yyyy-MM-dd") & "','" & Trim(txtsearchs.Text & "") & "'"
            dt = db.sub_GetDatatable(strSql)
            grdcontainer.DataSource = dt
            grdcontainer.DataBind()
            up_grid.Update()

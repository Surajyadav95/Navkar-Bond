...
Partial Class Summary_BCYMovement
    ...
    Protected Sub btncancel_Click(sender As Object, e As EventArgs)
        Try
            For Each row As GridViewRow In grdcontainer.Rows
                ...
                strSql = ""
                strSql += "USP_insert_IsApprovr_update " & Trim(chkright.Checked & "") & "," & CType(row.FindControl("lblentryid"), Label).Text & "," & Session("UserId_BondCFS")
    ...
    Protected Sub btnSearch_Click(sender As Object, e As EventArgs)
        Try
            strSql = ""
            strSql += " usp_Approve_tariff '" & Trim(ddltraiff.SelectedItem.Text & "") & "','" & Trim(ddlbondType.SelectedItem.Text & "") & "'"
            dt = db.sub_GetDatatable(strSql)
            grdcontainer.DataSource = dt
            grdcontainer.DataBind()
            up_grid.Update()

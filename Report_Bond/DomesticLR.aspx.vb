...
Partial Class Report_Estimation_Default
    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            If Not (Request.QueryString("Lrno") = "") Then
                LoadReport()
            End If
        End If
    End Sub
    ...
    Private Sub LoadReport()
        Try
            ...
            ReportViewer1.LocalReport.ReportPath = Server.MapPath("~/Report_Bond/DomesticLR.rdlc")

            Dim Lrno As String = Request.QueryString("Lrno")
            'Dim workyear As String = Request.QueryString("workyear")
           

            'strSql = ""
            'strSql += "get_imp_ind_charges '" & assessno & "','" & workyear & "'"
            'dt = db.sub_GetDatatable(strSql)

            strSql = ""
            strSql += "USP_LR_Print '" & Lrno & "'"
            ds = db.sub_GetDataSets(strSql)
            
            dt2 = ds.Tables(0)
            Dim datasource As New ReportDataSource("LrReceipt", dt2)
            'dt3 = ds.Tables(1)
            'Dim datasource1 As New ReportDataSource("AmountDetails", dt3)
            'dt4 = ds.Tables(2)
            'Dim datasource2 As New ReportDataSource("CargoAmount", dt4)
            'dt5 = ds.Tables(3)
            'Dim datasource3 As New ReportDataSource("RemarksHed", dt5)

...
Partial Class Summary_BCYMovement
    ...
    Protected Sub btnShow_Click(sender As Object, e As EventArgs)
        Try
            ...
            If ddlinvoicetype.SelectedItem.Text = "Empty Repo" Then
                strSql = ""
                strSql = "SP_GetOutContainerDetails_EmptyRepo'" & Trim(ddlshipline.SelectedValue & "") & "','" & Convert.ToDateTime(Trim(txtFromDate.Text & "")).ToString("yyyy-MM-dd HH:mm") & "','" & Convert.ToDateTime(Trim(txtBilledToDate.Text & "")).ToString("yyyy-MM-dd HH:mm") & "','" & Trim(txtcontainer.Text & "") & "','" & Session("UserId_DepoCFS") & "','" & Trim(ddlVessels.SelectedItem.Text & "") & "'"
                dt1 = db.sub_GetDatatable(strSql)
                grdcontainer.DataSource = dt1
                grdcontainer.DataBind()
                UpdatePanel1.Update()
        ...
        Try
                ...
                For Each row As GridViewRow In grdcontainer.Rows
                    ...
                    If chkright.Checked = True Then
                        ...
                        If Not dt4.Rows.Count > 0 Then
                            ScriptManager.RegisterStartupScript(Me, Me.GetType, "Key", "alert('Container No " + Trim(CType(row.FindControl("lblContainerN"), Label).Text) + " And To Container Type " + Trim(CType(row.FindControl("lblContainerType"), Label).Text) + " And To Size " + Trim(CType(row.FindControl("lblSize"), Label).Text) + "  tariff Not Added.');", True)
                            Exit Sub
                        End If
                    End If
                Next
            ...
            For Each row As GridViewRow In grdcontainer.Rows
                ...
                If chkright.Checked = True Then
                    If Not (Trim(ddlinvoicetype.SelectedItem.Text & "") = "Empty Container Repair" Or Trim(ddlinvoicetype.SelectedItem.Text & "") = "Washing") Then
                        strSql = ""
                        strSql = "USP_CALCULATION_STORAGE'" & Trim(ddltariff.SelectedItem.Text & "") & "','" & Convert.ToDateTime(Trim(txtInvoiceDate.Text & "")).ToString("yyyyMMdd") & "','" & Val(CType(row.FindControl("lblSize"), Label).Text) & "','" & Trim(ddlinvoicetype.SelectedItem.Text & "") & "','" & Trim(CType(row.FindControl("lblContainerType"), Label).Text) & "'"
                        ...
                        If dt5.Rows.Count > 0 Then
                            For j As Integer = 0 To dt5.Rows.Count - 1
                                ...
                                If dt6.Rows.Count > 0 Then
                                    Call sub_fetchcharges_IND(Trim(CType(row.FindControl("lblContainerN"), Label).Text), Trim(CType(row.FindControl("lblContainerType"), Label).Text), Val(CType(row.FindControl("lblSize"), Label).Text), Val(dt5.Rows(j)("AccountID")), 0, 0, 0, Trim(dt5.Rows(j)("deliverytype")), 0, Val(CType(row.FindControl("lblEntryID"), Label).Text), Val(CType(row.FindControl("lblTotalDays"), Label).Text), Trim(CType(row.FindControl("lblOutDate"), Label).Text), Trim(CType(row.FindControl("lblInDate"), Label).Text))
                        ...
                        End If
                    Else
                        Call sub_fetchcharges_IND(Trim(CType(row.FindControl("lblContainerN"), Label).Text), Trim(CType(row.FindControl("lblContainerType"), Label).Text), Val(CType(row.FindControl("lblSize"), Label).Text), 5, 0, 0, 0, Trim(ddlinvoicetype.SelectedItem.Text), 0, Val(CType(row.FindControl("lblEntryID"), Label).Text), Val(CType(row.FindControl("lblTotalDays"), Label).Text), Trim(CType(row.FindControl("lblOutDate"), Label).Text), Trim(CType(row.FindControl("lblInDate"), Label).Text))
                        If Val(dblNetAmount_IND) > 0 Then
                            dblGroup1Amt = dblGroup1Amt + dblNetAmount_IND
                            strSql = ""
                            strSql += "USP_Insert_temp_bill_emptyD '0' ,'" & strWorkyear & "','" & Val(CType(row.FindControl("lblEntryID"), Label).Text) & "' , "
                            ...
                            db.sub_ExecuteNonQuery(strSql)
                        End If
                        Call sub_fetchcharges_IND(Trim(CType(row.FindControl("lblContainerN"), Label).Text), Trim(CType(row.FindControl("lblContainerType"), Label).Text), Val(CType(row.FindControl("lblSize"), Label).Text), 6, 0, 0, 0, Trim(ddlinvoicetype.SelectedItem.Text), 0, Val(CType(row.FindControl("lblEntryID"), Label).Text), Val(CType(row.FindControl("lblTotalDays"), Label).Text), Trim(CType(row.FindControl("lblOutDate"), Label).Text), Trim(CType(row.FindControl("lblInDate"), Label).Text))
                        If Val(dblNetAmount_IND) > 0 Then
                            dblGroup1Amt = dblGroup1Amt + dblNetAmount_IND
                            strSql = ""
                            strSql += "USP_Insert_temp_bill_emptyD '0' ,'" & strWorkyear & "','" & Val(CType(row.FindControl("lblEntryID"), Label).Text) & "' , "
                            ...
                            db.sub_ExecuteNonQuery(strSql)
                        End If
                        Call sub_fetchcharges_IND(Trim(CType(row.FindControl("lblContainerN"), Label).Text), Trim(CType(row.FindControl("lblContainerType"), Label).Text), Val(CType(row.FindControl("lblSize"), Label).Text), 7, 0, 0, 0, Trim(ddlinvoicetype.SelectedItem.Text), 0, Val(CType(row.FindControl("lblEntryID"), Label).Text), Val(CType(row.FindControl("lblTotalDays"), Label).Text), Trim(CType(row.FindControl("lblOutDate"), Label).Text), Trim(CType(row.FindControl("lblInDate"), Label).Text))
                        If Val(dblNetAmount_IND) > 0 Then
                            dblGroup1Amt = dblGroup1Amt + dblNetAmount_IND
                            strSql = ""
                            strSql += "USP_Insert_temp_bill_emptyD '0' ,'" & strWorkyear & "','" & Val(CType(row.FindControl("lblEntryID"), Label).Text) & "' , "
    ...
    Protected Sub sub_fetchcharges_IND(ByVal ContainerNo As String, ByVal strContainerType As String, ByVal intContainerSize As Integer, ByVal strAccountID As String, ByVal dblWeight As Double, ByVal Strlocationfrom As String, ByVal Strlocationto As String, ByVal activity As String, ByVal ConDesc As String, ByVal movementno As String, ByVal TotalDays As String, strOutDate As String, strInDate As String)
        ...
        Try
            ...
            strSql += "'" & activity & "','" & strAccountID & "','" & Trim(ContainerNo & "") & "','" & movementno & "','" & strContainerType & "','" & Val(ddlshipline.SelectedValue) & "'"
            ...
            If dsFetch.Tables(1).Rows.Count > 0 Then
                ...
                '    dblAmount = 0
                'End If
                'If dsFetch.Tables(1).Rows(0)("isSQM") = True Then
                '    dblAmount = dblAmount * Val(dblSQM)
                'End If
    ...
    Protected Sub btnsave_Click(sender As Object, e As EventArgs) Handles btnsave.Click
        Try
            ...
            strSql += ",'" & Trim(ddlVessels.SelectedItem.Text) & "','" & Trim(txtPONo.Text & "") & "','" & Trim(txtViaNo.Text & "") & "','" & Convert.ToDateTime(txtBilledToDate.Text).ToString("yyyy-MM-dd HH:mm") & "','" & Trim(ddlreference.SelectedValue & "") & "','" & Trim(txtReference.Text & "") & "','" & Trim(txtdiscount.Text & "") & "'"
            dt = db.sub_GetDatatable(strSql)
            ...
            For Each row In grdcontainer.Rows
                ...
                If chkright.Checked = True Then
                    ...
                    strSql += "Containersize='" & Trim(CType(row.FindControl("lblSize"), Label).Text) & "' AND deliverytype='" & Trim(ddlinvoicetype.SelectedItem.Text & "") & "' " ' and ContainerType='" & Trim(grdcontainers.Rows(i).Cells("Type").Value) & "'" 'AND locfrom='" & Trim(grdcontainers.Rows(k).Cells("Location(From)").Value) & "' and locTo='" & Trim(grdcontainers.Rows(k).Cells("Location(To)").Value) & "'"
                    dt = db.sub_GetDatatable(strSql)
                    If (Trim(ddlinvoicetype.SelectedItem.Text & "") = "Empty Container Repair" Or Trim(ddlinvoicetype.SelectedItem.Text & "") = "Washing") Then
                        AccountID = 9
                    Else
